...
Microarrays Database (M3D)](http://www.m3d.mssm.edu), and explore some basic
...
Before beginning, let's first load a few helper functions that will be useful
...
# Plot sample heatmap
#
plot_sample_heatmap = function (counts, design, metric='dist', col='heat.colors') {
    ...
    # Select plot colors
    plot_colors = sample_plot_colors(design)
    ...
    legend(x="topleft", legend=unique(design$Condition),
           col=unique(plot_colors$condition), pch=15)
    legend(x="topright", legend=unique(design$Batch),
...
#
plot_sample_pca = function(counts, design, main="", axis1=1, axis2=2,
                           include_table=TRUE) {
    # PCA
    pca = makeSVD(counts)
    ...
    pcaData = data.frame(SampleID=colnames(counts),
                         ...
                         Condition=design$Condition, Batch=design$Batch)
    ...
    # condition
    if (nlevels(design$Batch) > 1) {
        pcs = pcRes(pca$v, pca$d, design$Condition, design$Batch)
    } else {
        pcs = pcRes(pca$v, pca$d, design$Condition)
    }
